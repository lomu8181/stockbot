...
def GetOhlcv(area, stock_code, limit = 500, adj_ok = "1"):
    ...
    try:
        ...
        if area == "US":
            ...
            if GetNowDist() == "VIRTUAL":
                print("----First try----")
                df = KisUS.GetOhlcv(stock_code,"D",adj_ok) #새로 대체!

                #한투에서 1000보다 크면 백테스팅용일 확률이 크니깐.
                if df is None or len(df) == 0 or Adjlimit > 100:
                ...
                print("----First try----")
                df = KisUS.GetOhlcvNew(stock_code,"D",limit,adj_ok) #새로 대체!
            ...
            if GetNowDist() == "VIRTUAL":
                ...
                print("----First try----")
                df = KisKR.GetOhlcv(stock_code,"D",adj_ok)

                #한투에서 100개 이상 못가져 오니깐 그 이상은 아래 로직을 탄다. 혹은 없는 종목이라면 역시 아래 로직을 탄다
                if Adjlimit > 100 or df is None or len(df) == 0:
                    ...
                    if except_riase == True:
                        ...
                        if df is None or len(df) == 0:
                            df = KisKR.GetOhlcvNew(stock_code,"D",limit,adj_ok) 
...
def GetOhlcv2(area, stock_code, limit = 500, adj_ok = "1"):
    ...
    if area == "KR":
        ...
        df = yfinance.download(stock_code, period='10y')
